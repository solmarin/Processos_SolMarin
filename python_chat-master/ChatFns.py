...
def LoadOtherEntry(ChatLog,user,EntryText):
    if EntryText != '':
        ChatLog.config(state=NORMAL)
        if ChatLog.index('end') != None:
            try:
                ...
                pass
            ChatLog.insert(END,user + EntryText)
            ChatLog.tag_add(user, LineNumber, LineNumber+float(len(user))/10)
            ChatLog.tag_config(user,foreground="#04B404", font=("Arial", 12, "bold"))
            ChatLog.config(state=DISABLED)
            ChatLog.yview(END)
